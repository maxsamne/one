...
def _row_to_schedule(row: tuple) -> Schedule:
    return Schedule(
        ...
        enabled=bool(row[5]),
        created_at=row[6], last_run_at=row[7],
        mode=row[8] if len(row) > 8 else None,
        graders=json.loads(row[9] or "[]") if len(row) > 9 else [],
    )
...
_SELECT = "SELECT id, cron, prompt, tier, skills_json, enabled, created_at, last_run_at, mode, graders_json"


def create(*, cron: str, prompt: str, tier: str = "ultra_cheap",
           skills: list[str] | None = None, graders: list[str] | None = None,
           enabled: bool = True, mode: str | None = None) -> Schedule:
    validate_cron(cron)
    _validate_mode(mode)
    sched = Schedule(
    ...
    with _lock:
        _get_con().execute(
            """INSERT INTO schedules
                   (id, cron, prompt, tier, skills_json, graders_json, enabled, created_at, last_run_at, mode)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
...
def update(sched_id: str, **fields) -> Schedule | None:
    """Partial update. Recognised fields: cron, prompt, tier, skills, graders, enabled, last_run_at, mode."""
    if not fields:
        return get(sched_id)
    if "cron" in fields:
